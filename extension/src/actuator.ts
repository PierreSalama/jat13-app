...
import type { Cmd, CmdResult, PageSnapshot, TargetRef, WaitCond } from './protocol.js';
...
/** Snapshot after a mutating op — the app decides the next move against this. buildSnapshot always
 *  returns a real snapshot (never undefined), so this is a concrete PageSnapshot the result carries. */
function delta(ctx: ActuatorCtx): PageSnapshot {
