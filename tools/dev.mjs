// `npm run dev` — build once, then launch Electron with the dev identity (port 7861, userData
// jat13-app-dev). NOTE the native-ABI dance: vitest runs better-sqlite3 under NODE's ABI, Electron
// needs ELECTRON's ABI. This launcher rebuilds for Electron first; after a dev session, run
// `npm run rebuild:node` before `npm test`. (M1 will pin this via an electron-based test runner.)
...
const child = spawn(electron, [join(ROOT, 'app')], {
  stdio: 'inherit',
