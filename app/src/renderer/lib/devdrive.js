// Dev-drive poller (renderer half) — ported verbatim from the proven 13.0.x
// harness (git cb25d19). Active ONLY when the app reports devtools on. Drains
// queued commands from /api/dev/pending, runs them against the REAL DOM (so the
// visible window moves), and posts results back. This is the test channel: it
// lets the app be driven over the loopback instead of stealing the user's mouse.
// No product behavior depends on it. NOTE: /api/dev/* speaks the raw command
// shape ({commands:[{id,type,args}]}), not the {ok,data} envelope — it is part
// of the harness contract with tools/dev-drive, kept byte-compatible on purpose.
  ...
  async function loop() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const commands = ((j && j.data) || j || {}).commands || []; // server envelopes /dev/pending; tolerate raw too
