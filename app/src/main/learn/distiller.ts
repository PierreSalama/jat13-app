...
// profile. It is the app-side half of watch-and-learn.
//
// SECURITY: redaction is enforced in TWO independent places and this is the second. The recorder never
// sends a sensitive VALUE (it marks the event `redacted`); this distiller DROPS every redacted event
// before it can be recorded; and even a non-redacted event with a sensitive LABEL is dropped a THIRD
// time by dal.answers.record() (isSensitiveKey → returns null). Defense in depth: a demographic /
...
export function makeLearnDistiller(deps: { dal: Dal }) {
  ...
  /** The default profile's id (learning always attaches to the default profile). */
  function defaultProfileId(): string | undefined {
    const row = dal.ctx.db.prepare('SELECT id FROM profiles WHERE is_default = 1 LIMIT 1').get() as
      | { id: string }
      | undefined;
    return row?.id;
  }
  ...
  function ingest(batch: ObservedBatch): IngestResult {
    ...
    if (learned > 0 && host) {
      try {
        dal.events.record({ kind: 'note', source: host.slice(0, 64), summary: `Learned ${learned} answer(s) from you on ${host}` });
      } catch {
