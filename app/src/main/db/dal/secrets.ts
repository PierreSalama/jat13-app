...
// Stage 0 uses it for exactly one credential: the extension pairing token (chrome side stores it
// under "jat13Token"; the app side keeps ITS copy sealed here). Gmail OAuth + future creds reuse
...
// at-rest form is the OS-sealed BLOB in `secrets.sealed`. safeStorage (the real Sealer) is
// unavailable under vitest, so the DAL takes an INJECTED Sealer — production wires Electron
// safeStorage (DPAPI on Windows), tests wire a fake. The DAL treats the sealed bytes as opaque and
// only round-trips them through the Sealer.
//
// This is the ONE writer of the `secrets` table. Every mutating write emits a DomainEvent whose
// patch carries STATUS/health fields only — never the plaintext, never the sealed bytes.

import type { DalContext, DomainEvent } from './index.js';
import { makeStmtCache } from './index.js';

/**
 * The sealing primitive the DAL depends on. Production passes an adapter over Electron
 * `safeStorage` (OS keychain / DPAPI); tests pass a fake. `available()` is false when the platform
 * can't seal (e.g. a headless box with no keyring, or vitest) — callers must refuse to store
...
/** The surface the rest of the app codes against (pairing / Gmail sync / token-health UI). */
...
export function makeSecretsDal(ctx: DalContext, sealer: Sealer): SecretsDal {
  ...
  /**
   * Seal `plaintext` under `key` and store the opaque BLOB with status='ok'. Refuses (throws) if
   * the platform can't seal — we NEVER persist plaintext as a fallback. The emitted patch carries
   * only the health fields; it must never contain the plaintext or the sealed bytes.
   */
  ...
  /**
   * Read + unseal the credential for `key`, or `undefined` if there is no row. The plaintext
   * returned here is the ONLY place it exists in memory — the caller must not persist it.
   */
  ...
  /**
   * A consumer reports the outcome of USING the credential. On success: status='ok',
   * last_ok_at=now, last_error cleared. On failure: status='expired' (default, refreshable) or
   * 'revoked' (dead) per `reason`, last_error captured (capped). `expiresHintAt` (if given) is
   * always recorded. No-op if the key doesn't exist (never resurrects a deleted secret).
   * Never touches `sealed`.
   */
  ...
  /**
   * The token-health UI feed. Explicit health-only column list — `sealed` is never selected, so
   * this endpoint structurally cannot leak the credential bytes. Ordered by key for a stable
   * panel; small fixed set of keys, so no limit is needed.
   */
