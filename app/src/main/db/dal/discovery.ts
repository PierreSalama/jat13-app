// discovery DAL — the four supply-lane tables (discovery_sources, company_tokens, discovery_batches,
// job_sightings). This is the anti-starvation surface: every lane is its OWN discovery_sources row with
// its OWN pacing gate (next_earliest_at) + breaker (cooldown_until) + cursor, so nothing shared can let
// one lane starve another (the v11.83 shared-refill-gate scar is structurally impossible here).
//
// Ported from the proven cb25d19 tree; re-pointed at this tree's ./index.js foundation (makeStmtCache /
// clampLimit live there, not in a ./util.js) and bound to migration 001_init columns verbatim.
//
// The PERMANENT-DISMISS surface (dismissals + jobs.dismissed_at — Pierre's 2026-07-10 scar) lives in the
// sibling db/dal/dismissals.ts (makeDismissalsDal), NOT here: it is a jobs-level concern shared by the
// /track route + the discovery ingest path. The discovery ingest chokepoint (discovery/ingest.ts) consults
// that DAL's isDismissed() before every upsert.
//
// Structural laws enforced HERE in TypeScript, belt-to-the-DDL's-CHECK-suspenders:
...
//      NOTHING; the service simply never calls recordBatch on a dry scan (yield-only telemetry).
...
//
// snake_case DTOs (matching jobs/events), parameterized SQL only, one prepared-statement cache per module,
// a DomainEvent on every batch insert + source change.

import type { DalContext } from './index.js';
import { makeStmtCache, clampLimit } from './index.js';

// ---- vocabularies (mirror the migration 001 CHECKs) ------------------------------------------------
...
/** board → (deterministic source id, kind). ids match the §2.8 examples ('src_gh' for greenhouse). The
 *  jobspy/extension lanes are seeded too so their pacing gate + breaker + cursor have a home from tick 1. */
const BOARD_META: Record<Board, { id: string; kind: SourceKind }> = {
  linkedin: { id: 'src_linkedin', kind: 'jobspy' },
...
/** every lane board id, in a stable order (used by the service's per-lane scheduler). */
export const ALL_BOARDS = ['linkedin', 'indeed', 'greenhouse', 'lever', 'ashby'] as const satisfies readonly Board[];
...
export function makeDiscoveryDal(ctx: DalContext) {
  ...
  function sourceById(id: string): DiscoverySource | undefined {
    const row = stmt(`SELECT ${SOURCE_COLS} FROM discovery_sources WHERE id = ?`).get(id) as
      | SourceRow
      | undefined;
    return row ? toSource(row) : undefined;
  }
