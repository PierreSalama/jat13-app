...
import { makeStmtCache, clampLimit, clampOffset, normKey, normJobUrl } from './index.js';
...
export function makeJobsDal(ctx: DalContext) {
  ...
  function markSeen(
    id: string,
    opts: { capability?: ApplyCapability } = {},
  ): JobLean | undefined {
    // NOTE: per-source sighting provenance lives in `job_sightings` (schema 001), which is the
    // DISCOVERY lane's write (Stage 3) — a discovery DAL upserts the (job_id, source_id) row there.
    // markSeen only freshens the posting itself; it never touches external_id (clobbering it would
    // corrupt the (source, external_id) dedup index).
    const ts = now();
...
export type JobsDal = ReturnType<typeof makeJobsDal>;
