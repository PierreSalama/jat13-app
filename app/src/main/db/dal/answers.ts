...
// ever asks the human. Ported from the proven cb25d19 tree; binds to migration 001_init column names
// + CHECKs verbatim (the new schema adds provenance 'deterministic' — ranked below the human-sourced
// tiers, above 'ai').
//
...
import { makeStmtCache, clampLimit, clampOffset, normKey, normQuestion } from './index.js';

// ---- domain types ----------------------------------------------------------

export type AnswerKind = 'field' | 'qa';
export type FieldType = 'text' | 'textarea' | 'select' | 'radio' | 'checkbox' | 'number' | 'date' | 'file';
export type Provenance = 'user' | 'harvest' | 'ai' | 'teach' | 'profile_push' | 'deterministic' | 'import_v11';
...
// `deterministic` (derived from profile facts by rule) beats AI guesses but never a human tier.
...
export function makeAnswersDal(ctx: DalContext) {
  ...
  function list(profileId: string, input: ListInput = {}): LeanPage<LearnedAnswerLean> {
    ...
    if (input.q && input.q.trim()) {
      where.push("(label LIKE @q ESCAPE '\\' OR key_norm LIKE @q ESCAPE '\\')");
      // escape LIKE metacharacters so a user typing "50%" isn't a wildcard search.
      const safe = input.q.trim().replace(/[\\%_]/g, (m) => '\\' + m);
      params.q = `%${safe}%`;
    }
...
// The lean patch a mutation broadcasts on the event sink: the changed row in its list shape (no
// value/options text). Returned as a plain Record so it satisfies DomainEvent.patch's index signature.
...
export type AnswersDal = ReturnType<typeof makeAnswersDal>;
