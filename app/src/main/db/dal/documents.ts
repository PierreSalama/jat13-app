// documents DAL — bytes live IN the database (a disk restore already cost one library). Three tables:
...
//   document_text  — extracted plaintext + keywords for matching/tailoring
//
...
// Ported from the proven cb25d19 tree, extended with migration 001's generated-doc lineage columns
// (derived_from / application_id / guardrail_hash / guardrail_status): AI-tailored docs share this
// table — 'parked' guardrail docs are never auto-attached, they sit in review.
...
export type DocumentRole = 'resume' | 'cover_letter' | 'portfolio' | 'transcript' | 'brief' | 'other';
/** documents.source vocabulary (CHECK in 001). */
export type DocumentSource = 'upload' | 'application' | 'folder' | 'generated' | 'import_v11';
/** Rephrase-only post-check verdict for generated docs; NULL = not a generated doc. */
export type GuardrailStatus = 'pending' | 'passed' | 'parked';
...
export interface AddDocumentInput {
  ...
  source?: DocumentSource;
  originPath?: string;
  // generated-doc lineage (AI tailoring; Stage 4 wires the writers, the columns exist now):
  derivedFrom?: string;
...
const LEAN_COLS =
  'id, profile_id, name, role, label, mime, size_bytes, sha256, is_default, source, origin_path, ' +
  'missing_file, derived_from, application_id, guardrail_hash, guardrail_status, created_at, updated_at';
...
function toLean(r: DocumentRow): DocumentLean {
  return {
    ...
    guardrail_hash: r.guardrail_hash,
...
export function makeDocumentsDal(ctx: DalContext) {
  ...
  function emit(op: DomainEvent['op'], id: string, patch?: Record<string, unknown>): void {
    const evt: DomainEvent =
      patch === undefined ? { table: 'documents', op, id } : { table: 'documents', op, id, patch };
    ctx.emit(evt);
  }
  ...
  function findBySha(sha256: string): DocumentRow | undefined {
    return stmt(`SELECT ${LEAN_COLS} FROM documents WHERE sha256 = ?`).get(sha256) as
      | DocumentRow
      | undefined;
  }

  /** True when this role already has a default document (so the first insert becomes the default). */
  function roleHasDefault(role: DocumentRole): boolean {
    const row = stmt('SELECT 1 AS x FROM documents WHERE role = ? AND is_default = 1 LIMIT 1').get(
      role,
    ) as { x: number } | undefined;
    return row !== undefined;
  }

  /** One document's lean metadata (never bytes/text). */
  function get(id: string): DocumentLean | undefined {
    const row = getRow(id);
   ...
   * return THAT document — no second blob is written (content-addressed dedup; the partial unique
   * index idx_documents_sha backs it). Otherwise insert the metadata row + the blob atomically.
   * First document of a role becomes that role's default — EXCEPT generated docs (a tailored
   * one-off must never silently become the library default).
   */
  function add(input: AddDocumentInput): DocumentLean {
    ...
    const source: DocumentSource = input.source ?? 'upload';
    const now = ctx.now();
    const id = ctx.newId('doc');
    const isDefault = source !== 'generated' && !roleHasDefault(role) ? 1 : 0;
    ...
    const insert = ctx.db.transaction((): DocumentRow => {
      ...
      stmt(
        `INSERT INTO documents
         ...
         VALUES
           (@id, @profile_id, @name, @role, @label, @mime, @size_bytes, @sha256, @is_default, @source, @origin_path,
            0, @derived_from, @application_id, @guardrail_hash, @guardrail_status, @created_at, @updated_at)`,
      ).run({
        id,
        ...
        guardrail_hash: input.guardrailHash ?? null,
        guardrail_status: input.guardrailStatus ?? null,
        created_at: now,
        updated_at: now,
      });
    ...
    stmt(
      `INSERT INTO documents
         (id, profile_id, name, role, label, mime, size_bytes, sha256, is_default, source, origin_path,
          missing_file, derived_from, application_id, guardrail_hash, guardrail_status, created_at, updated_at)
       VALUES
         (@id, @profile_id, @name, @role, NULL, NULL, 0, NULL, 0, 'import_v11', @origin_path,
          1, NULL, NULL, NULL, NULL, @created_at, @updated_at)`,
    ).run({
      id,
      profile_id: input.profileId ?? null,
      name: input.name,
  ...
  function setText(id: string, input: SetTextInput): void {
    ...
    const keywords = Array.isArray(input.keywords)
      ? input.keywords.filter((k) => typeof k === 'string')
      : [];
    const now = ctx.now();
    ctx.db.transaction(() => {
      stmt(
  ...
  /** Paged document metadata (NEVER bytes, NEVER extracted text), role-grouped with defaults first.
   *  `applicationId` drives the drawer's generated-docs panel; `source:'generated'` the Generated tab. */
  function listLean(opts: ListDocumentsInput = {}): LeanPage<DocumentLean> {
    ...
    if (opts.source !== undefined) {
      ...
      bind.source = opts.source;
    }
    if (opts.profileId !== undefined) {
      where.push('profile_id = @profileId');
      bind.profileId = opts.profileId;
    }
    if (opts.applicationId !== undefined) {
      where.push('application_id = @applicationId');
      bind.applicationId = opts.applicationId;
    }
    const clause = where.length ? `WHERE ${where.join(' AND ')}` : '';
    const total = (stmt(`SELECT COUNT(*) AS c FROM documents ${clause}`).get(bind) as { c: number }).c;
    const rows = stmt(
      `SELECT ${LEAN_COLS} FROM documents ${clause}
       ORDER BY role ASC, is_default DESC, updated_at DESC LIMIT @limit OFFSET @offset`,
    ).all({ ...bind, limit, offset }) as DocumentRow[];
   ...
   * it on this one. Atomic, so the "exactly one default per role" invariant never briefly breaks.
   */
  function setDefault(id: string): void {
    ...
    ctx.db.transaction(() => {
      stmt(
        'UPDATE documents SET is_default = 0, updated_at = @now WHERE role = @role AND is_default = 1',
      ).run({ now, role: doc.role });
...
export type DocumentsDal = ReturnType<typeof makeDocumentsDal>;
