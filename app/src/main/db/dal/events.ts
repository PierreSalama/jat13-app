// events DAL — the durable, user-visible timeline (NOT a ring buffer; discovery_batches/ai_calls are
// the rings). Every user-meaningful thing that happens to a job/application/run/email lands here
// exactly once and stays: status changes, submits, parks, matched emails, tailored docs, interviews,
// autopsies, notes, imports. The Activity page and the per-application drawer both read this table.
// Append-only by contract — there is no update/delete surface (the only mutation is INSERT), so a
// row, once recorded, is history. Ported from the proven cb25d19 tree; kind vocabulary reconciled to
// the FULL migration-001 CHECK (the old tree's 8 kinds grew to 15 — the schema is the authority).
...
/** events.data_json CHECK caps the serialized column at 4096 bytes. Anything strictly larger is
 *  dropped and replaced by a warning marker so the timeline never silently loses the row just
 *  because its payload was fat. */
...
export function makeEventsDal(ctx: DalContext) {
   ...
   * past the 4096-byte cap. Emits a DomainEvent { table:'events', op:'insert', id, patch } so a live
   * consumer can push the new row without a refetch.
   */
   ...
   * index. Default limit 200 (paged via offset); total is the unbounded count for that application.
   */
  function timeline(
    applicationId: string,
    ...
    const rows = (
      stmt(
        `SELECT ${SELECT_COLS} FROM events WHERE application_id = ? ORDER BY at DESC, id DESC LIMIT ? OFFSET ?`,
      ).all(applicationId, limit, offset) as RawEventRow[]
    ).map(toRow);
   ...
   * the Activity page read. Default limit 100. Unknown kinds in the filter are dropped (they can
   * never match a stored row); an empty-after-filtering kind list yields an empty page rather than
   * an unfiltered scan.
   */
  function recent(
    opts: { limit?: number; offset?: number; kinds?: readonly string[] } = {},
  ): LeanPage<EventRow> {
    ...
    if (opts.kinds !== undefined) {
      ...
      const rows = (
        stmt(
          `SELECT ${SELECT_COLS} FROM events WHERE kind IN (${placeholders}) ORDER BY at DESC, id DESC LIMIT ? OFFSET ?`,
        ).all(...kinds, limit, offset) as RawEventRow[]
      ).map(toRow);
      const { total } = stmt(
    ...
    const rows = (
      stmt(`SELECT ${SELECT_COLS} FROM events ORDER BY at DESC, id DESC LIMIT ? OFFSET ?`).all(
        limit,
        offset,
      ) as RawEventRow[]
    ).map(toRow);
