// The applications aggregate — one row per (job, profile), the canonical status lifecycle.
...
// `funnel` is THE one funnel authority — Pipeline counts, Command-Center numbers, everything reads it.
// Ported from the proven cb25d19 tree; binds to migration 001_init columns verbatim.
...
/** getDetail result — lean fields PLUS the JSON columns hydrated (parsed) + notes. Loaded one row at
 *  a time by the detail drawer, never shipped in a list. */
export interface ApplicationDetail extends ApplicationLean {
  answers: unknown[];
  attachments: unknown[];
  notes: string | null;
}
...
export function makeApplicationsDal(ctx: DalContext) {
  ...
  function elevate(id: string, status: ApplicationStatus, via?: ApplicationVia): ApplicationRow {
    ...
    return db.transaction((): ApplicationRow => {
      ...
      // 1) same status → no-op (elevate is status-only; use patch for `via`-only updates).
  ...
  /** One application with its JSON columns hydrated — the detail-drawer read. */
  function getDetail(id: string): ApplicationDetail | undefined {
    const row = getById(id);
  ...
  /**
   * THE funnel authority: count applications by status within a trailing window
   * (updated_at >= now - days*86.4e6). Every status key is present (zero-filled) so a caller can
   * render a stable funnel shape. Pipeline columns, Command-Center counts — everything reads THIS.
   */
  ...
  return { ensure, elevate, patch, get: getById, getDetail, listLean, funnel };
}

// ---- helpers (module-local, no state) --------------------------------------------------------------

/**
 * Project a full row to its lean shape for the event-sink payload (never ships heavy text columns).
...
/** Defensive parse for a JSON-array column (CHECK-validated on write; still never crash a read). */
...
export type ApplicationsDal = ReturnType<typeof makeApplicationsDal>;
