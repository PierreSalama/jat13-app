// The one writer. better-sqlite3, WAL, main-process only (single-writer law).
// Every other subsystem reaches SQLite through the DAL modules that sit on top of this handle —
...
export interface OpenOptions {
  ...
  file: string;
  /** Override the migrations directory (packaged app passes an explicit path beside main.js). */
  migrationsDir?: string;
  /** Open read-only (the v11 importer opens the SOURCE copy this way; never the live v11 file). */
  readonly?: boolean;
}
...
/**
 * Pragmas applied to every writable handle. Kept in one place so diagnostics can assert them and a
 * stray connection can never run with the wrong durability/enforcement.
 ...
 * Throws before returning if the DB is from a newer app, a migration fails, or the migrations dir
 * holds an unrecognized .sql — the caller surfaces that as the read-only "migration failed /
 * update the app" launch state instead of running on a half-understood schema.
 */
export function openDatabase(opts: OpenOptions): OpenResult {
  ...
  if (readonly) {
    ...
    return { db, migration: { from, to: from, applied: [], backups: [] } };
  }
