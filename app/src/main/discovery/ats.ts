// ats.ts — public-JSON-board fetch + parse + gates for the three high-conversion ATSes. Ported from v11
// discovery/ats-boards.js (the module that fixed Greenhouse/Lever/Ashby sitting STARVED at ~0 done because
// nothing ever queried their public boards). No scraping, no API key: each ATS exposes an unauthenticated
// JSON endpoint that returns EVERY role at a company.
//
...
// query already constrains keyword+location), the two positive gates here are load-bearing: without them a
// Canada user's queue floods with SF/London/Bengaluru roles (v11 measured ~400 overseas "applyable" jobs
// from the first two ticks). `fetchImpl` is injected so tests feed canned JSON and real network is never hit.
...
/** A normalized posting — already in the shape the jobs DAL upsert consumes (plus `remote`, which the
 *  location gate needs and jobs.upsert ignores). apply_capability is always 'ats_form': these are
 *  hosted ATS application forms, which pillar-2's greenhouse/lever/ashby adapters drive. */
