// The adapter registry — the app's map from a URL to the recipe that drives it (Pillar 3 §4). For M1
// this loads validated builtins straight off disk; DB-backed versioning / hot-reload / rollback (§4.2)
// layer on later. Two laws hold from day one:
...
//   2. Routing is by host GLOB + priority only (never CSS/class) — the highest-priority adapter whose
//      hosts[] matches the URL's host wins; ties break by declaration order.
...
// builtins live beside THIS module (src/main/adapters/builtin). tests + `npm run dev` resolve relative
// to the source; the packaged app passes an explicit dir (build.mjs copies builtin/ beside the bundle),
 ...
 * matches any run of chars including none, so `*.linkedin.com` matches `www.linkedin.com` AND the apex
 * `linkedin.com`. Every other char is escaped — a glob is data, never a source of regex injection.
 */
 ...
 * or a URL with no host resolves to null — the caller then takes the generic-fallback path.
 */
